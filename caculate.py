...
def total_test():
    file_path="./test/model6_dis_dur_energy_area_size_sdr.txt"#读取划分好的文件
...
def globle_nor_weight():
    inpath="./GradientShap_weight1.txt"
    outpath = inpath.replace("GradientShap", "nor")
    max=0
    min=0
    for i in range(2,7):
...
if __name__ == '__main__':
    dot_path=["model1_dur","model2_dur_energy","model3_dur_energy_sdr","model4_dur_energy_size_sdr",
    ...
    input_path="./GradientShap_weight1.txt"
    output_path=input_path.replace("GradientShap","nor")
    # total_test()
    # nor_weight(input_path,output_path)
    # globle_nor_weight()
    # globle_caculate()
    a=1